...
void MT_NlmeStartRouterRequest(uint8 *pBuf)
{
  ...
  if ( ZSTACK_ROUTER_BUILD )
  {
    retValue = (byte)NLME_StartRouterRequest(pBuf[0], pBuf[1], pBuf[2]);
  }
  else
  {
    retValue = ZUnsupportedMode;
...
void MT_NlmeRouteDiscoveryRequest(uint8 *pBuf)
{
  ...
  if ( ZSTACK_ROUTER_BUILD )
  {
    retValue = NLME_RouteDiscoveryRequest(BUILD_UINT16(pBuf[0], pBuf[1]), 0, pBuf[2]);
  }
  else
  {
    retValue = ZUnsupportedMode;
...
void MT_NlmeOrphanJoinRequest(uint8 *pBuf)
{
  ...
  if ( ZSTACK_END_DEVICE_BUILD )
  {
    ...
    if (j == 1)
    {
      ...
      retValue = (byte)NLME_OrphanJoinRequest(channelList, *pBuf);
    }
    else
    {
 ...
 * @return      none
 ***************************************************************************************************/
void nwk_MTCallbackSubDataIndication(uint16 SrcAddress, int16 nsduLength, byte *nsdu, byte LinkQuality)
{
  byte *msgPtr;
  byte *msg;
  byte msgLen;
