/**************************************************************************************************
  ...
  NON-INFRINGEMENT AND FITNESS FOR A PARTICULAR PURPOSE. IN NO EVENT SREED
...
static uint8 ReedStatusOld;
static uint8 ReedStatusNow;
/*********************************************************************
 * LOCAL FUNCTIONS
 */
...
UINT16 SerialApp_ProcessEvent( uint8 task_id, UINT16 events )
{
    ...
    while ( (MSGpkt = (afIncomingMSGPacket_t *)osal_msg_receive( SerialApp_TaskID )) )
    {
      ...
      case ZDO_STATE_CHANGE:
          ...
          if(SampleApp_NwkState == DEV_END_DEVICE) //判定当前设备类型
          {
            ...
            osal_set_event(SerialApp_TaskID, REED_READ_EVT); //启动光照检测
          }
        break;
      default:
        break;
  ...
  if ( events & REED_READ_EVT )  //查看光敏电阻状态
  {
    ReedStatusOld = ReedStatusNow;  //保存当前状态
    if(REED_GPIO == LOW)
    {
     ReedStatusNow = REED_YES;
    }
    else
    {
      ReedStatusNow = REED_NO;
    ...
    if(ReedStatusNow != ReedStatusOld) //如果当前状态和上一个状态发生变化，发送消息给网关
    {
      UART0_Format.Command = 0x01;
      if(ReedStatusNow == REED_YES)
      {
    ...
    osal_start_timerEx(SerialApp_TaskID, REED_READ_EVT, 500);   
    return ( events ^ REED_READ_EVT );
  }
